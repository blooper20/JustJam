...
def generate_bass_tab(file_path):
    ...
    if not bass_notes:
        print(
            "Warning: No bass notes detected in 'bass' stem. "
            "Trying to infer from low frequencies..."
        )
    ...
    generator = TabGenerator(tuning=bass_tuning, bpm=bpm, slots_per_measure=16)
