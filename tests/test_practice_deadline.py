...
from src.api.models import ProjectModel, Team, TeamMember, ProjectMember
...
def test_update_deadline_permission(client, auth_headers, test_user, db):
    ...
    response = client.patch(
        ...
        json={"practice_deadline": "2026-12-31"},
    ...
    response = client.patch(
        f"/projects/{project_id}",
        json={"practice_deadline": "2026-06-01"},
    ...
    response = client.patch(
        f"/projects/{project_id}",
        json={"practice_deadline": "2026-07-01"},
        headers=other_headers
    )
    assert response.status_code == status.HTTP_200_OK
