...
def test_generate_project_tab(client, auth_headers, test_user, db):
    """프로젝트 타브 악보 생성 API 테스트"""
    ...
    project = ProjectModel(
        id=project_id,
        name="Tab Target",
        original_filename="tab_target.mp3",
        user_id=test_user.id,
        status="completed",
        bpm=120,
    )
    ...
    # UPLOAD_DIR에 임시 파일 준비
    upload_dir = os.path.join(os.getcwd(), "temp", "uploads")
    os.makedirs(upload_dir, exist_ok=True)
    with open(os.path.join(upload_dir, "tab_target.mp3"), "wb") as f:
        f.write(b"dummy audio content")

    # transcribe_audio 모킹
    mock_notes = [{"start": 0.0, "end": 0.5, "pitch": 60, "velocity": 0.8, "role": "melody"}]
    with patch(
        "src.api.services.project_service.transcribe_audio", return_value=(mock_notes, 120.0)
    ):
        response = client.post(f"/projects/{project_id}/tabs/guitar", headers=auth_headers)

    # clean up
    if os.path.exists(os.path.join(upload_dir, "tab_target.mp3")):
        os.remove(os.path.join(upload_dir, "tab_target.mp3"))
