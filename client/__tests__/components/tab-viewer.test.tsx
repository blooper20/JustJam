...
describe('TabViewer', () => {
  ...
  it('초기 상태에는 생성 버튼을 표시한다', () => {
    ...
    expect(screen.getByText(/guitar Tab Not Generated/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /생성하기 \(Guitar\)/i })).toBeInTheDocument();
  });

  it('탭 전환이 정상적으로 동작한다', async () => {
    render(<TabViewer projectId={projectId} />);

    const bassTabTrigger = screen.getByRole('tab', { name: /베이스 타브/i });
    fireEvent.keyDown(bassTabTrigger, { key: 'Enter', code: 'Enter', keyCode: 13 });
    fireEvent.click(bassTabTrigger);

    await waitFor(() => {
      expect(screen.getByText(/bass Tab Not Generated/i)).toBeInTheDocument();
    });
  });
  ...
  it('복사 버튼 클릭 시 클립보드 API를 호출한다', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/복사/i)).toBeInTheDocument());

    fireEvent.click(screen.getByText(/복사/i));
