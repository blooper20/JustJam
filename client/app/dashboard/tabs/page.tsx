...
import { Activity, Loader2, ChevronLeft, Clock } from 'lucide-react';
    ...
    return (
        <div className="container mx-auto p-8 space-y-8">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">보유 타브악보 (Tab)</h1>
                    <p className="text-muted-foreground">Guitar/Bass 타브악보를 확인하고 연습하세요</p>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projects.map((project) => (
                        <Link
                            key={project.id}
                            href={project.status === 'completed' ? `/projects/${project.id}?view=tab` : '#'}
                            className={cn(
                        ...
                        >
                            <Card className={cn(
                                ...
                                project.status === 'completed' ? "hover:shadow-lg hover:-translate-y-1 hover:border-pink-500/50" : "grayscale border-zinc-900"
                            )}>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium truncate pr-4">{project.name}</CardTitle>
                                    <div className="flex items-center gap-2">
                                        <Activity className={cn("h-4 w-4", project.status === 'completed' ? "text-pink-500" : "text-zinc-600")} />
                                    </div>
                                </CardHeader>
                                <CardContent>
                                    <div className="flex items-center justify-between mt-2">
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {project.has_tab ? (
                                                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-pink-500/20 text-pink-400 border border-pink-500/30 font-bold uppercase flex items-center gap-1">
                                            ...
                                            {project.has_tab && (
                                                <p className="text-[10px] text-pink-500/80 font-medium">✨ 이미 생성된 타브가 있어 즉시 열립니다.</p>
                                            )}
                                        </div>
