...
export const downloadMix = async (id: string, volumes: Record<string, number>, bpm: number = 120, metronomeVolume: number = 0, startOffset: number = 0): Promise<{ url: string }> => {
    const response = await apiClient.post(`/projects/${id}/mix`, { volumes, bpm, metronome: metronomeVolume, start_offset: startOffset });
    const data = response.data;
