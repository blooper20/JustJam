...
export default function ProjectPage() {
    ...
    const { data: project, isLoading } = useQuery({
        ...
        refetchInterval: (query) =>
            query.state.data?.status === 'processing' ? 1000 : false, // Poll faster
    });
    ...
    if (isLoading) return <div className="p-8 text-center"><Loader2 className="animate-spin inline mr-2" /> 불러오는 중...</div>;
            ...
            <div className="mb-6">
                ...
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm">Created: {new Date(project.created_at).toLocaleString()}</p>
                    </div>
                    <div className="flex items-center gap-2 relative">
                        {/* Status Glow for Project Page */}

                        <span className={`relative px-3 py-1 rounded-full text-sm font-medium bg-black/80 z-10 ${project.status === 'completed' ? 'text-green-500' :
            ...
            {project.status === 'processing' && (
                <Card className="bg-zinc-900 border-zinc-800">
                    <CardContent className="pt-6 text-center py-12">
                        ...
                        <p className="text-muted-foreground mb-6">음원 분리 작업은 약 3~5분 정도 소요됩니다. 잠시만 기다려주세요.</p>
            ...
            {project.status === 'failed' && (
                <Card className="border-red-900 bg-red-900/10">
                    <CardContent className="pt-6 flex flex-col items-center">
                        ...
                        <p className="text-muted-foreground">작업 처리 중 오류가 발생했습니다. 서버 로그를 확인해주세요.</p>
                    </CardContent>
                </Card>
            )}
        </div>
