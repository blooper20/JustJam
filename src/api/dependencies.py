...
async def get_current_user(
    ...
    if payload is None:
        raise InvalidTokenError(detail="인증 정보를 확인할 수 없습니다 (유효하지 않은 토큰)")

    user_id: Optional[int] = payload.get("user_id")
    if user_id is None:
        raise InvalidTokenError(detail="인증 정보를 확인할 수 없습니다 (사용자 정보 부족)")
...
async def get_current_active_user(current_user: User = Depends(get_current_user)) -> User:
    ...
    if not current_user.is_active:
        raise AuthenticationError(detail="비활성화된 사용자입니다")
    return current_user
