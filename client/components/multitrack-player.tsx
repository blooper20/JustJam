...
// ==================== 메인 컴포넌트 ====================
export function MultiTrackPlayer({ stems, projectId, initialBpm }: MultiTrackPlayerProps) {
    // 재생 상태
    ...
    // 트랙 초기화
    useEffect(() => {
        const validStems = Object.entries(stems)
            .filter(([name, url]) => !!url && name !== 'master') as [string, string][];
    ...
    // 파형 스크러빙 - 탐색 위치 계산
    const calculateSeekPosition = useCallback((e: MouseEvent | React.MouseEvent) => {
    ...
    const handleDownloadMix = async () => {
        ...
        if (newWindow) {
            newWindow.document.write('<html><body style="background:#18181b;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;"><h2>믹스 생성 중... 잠시만 기다려주세요...</h2></body></html>');
        }
        ...
        try {
            ...
            const { url } = await downloadMix(projectId, volumes, bpm, downloadMetronomeVolume, startOffsetSeconds);
                            ...
                            <div
                                ...
                                className={cn(
                                    ...
                                    metronomeEnabled && currentBeat === i
                                        ? (i === 0 ? "bg-primary shadow-[0_0_8px_theme(colors.primary.DEFAULT)] scale-125" : "bg-zinc-300 shadow-[0_0_6px_rgba(255,255,255,0.8)] scale-110")
                                        : "bg-zinc-800"
                                )}
                            />
                        ))}
