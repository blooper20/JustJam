...
- [x] **프로젝트 관리 고도화**: 대시보드 내 프로젝트 **이름 변경(Rename)**, **검색(Search)**, **정렬(Sort)**, **복제(Duplicate/Clone)** 기능 추가
- [x] **플레이어 기능 강화**: **재생 속도 조절(0.5x~2.0x)**, **구간 반복(A-B Loop)**, **북마크(Bookmark)** 및 **키보드 단축키** 지원
- [x] **출력 및 공유 기능**: 타브/악보 뷰어 내 **PDF 인쇄 및 MusicXML 다운로드** 기능 강화
...
**작업 체크리스트**:
- [x] pytest 설정 완료 (conftest.py, fixtures)
- [x] JWT 단위 테스트 작성
- [x] 인증 API 통합 테스트 작성
- [x] 프로젝트 CRUD 테스트 작성 (복제 기능 포함)
- [x] Jest + React Testing Library 설정
- [x] 핵심 컴포넌트 테스트 작성 (LandingPage, DashboardPage 완료)
...
#### 3.1.2 프로젝트 관리 개선
- [x] 프로젝트 이름 변경 기능 (`PATCH /projects/{id}`)
- [x] 프로젝트 복제 기능 (`POST /projects/{id}/clone`)
- [x] 프로젝트 정렬/필터 옵션 (날짜, 이름)
- [x] 프로젝트 검색 기능 (q 파라미터 연동)
- [x] 프로젝트 썸네일/커버 이미지 (자동 스펙트로그램 생성 완료)

#### 3.1.3 플레이어 개선
- [x] 키보드 단축키 지원 (Space: 재생/일시정지, 방향키: 탐색)
- [x] 구간 반복 재생 (A-B 루프)
- [x] 재생 속도 조절 (0.5x ~ 2.0x)
- [x] 북마크 기능 (특정 구간 저장 및 이동)

#### 3.1.4 악보/타브 뷰어 개선
- [x] 악보 줌 인/아웃
- [x] 인쇄/PDF 저장 기능 (브라우저 인쇄 연동)
- [x] 타브 뷰어에서 현재 재생 위치 하이라이트 (MusicXML 커서 연동)
- [x] 악보 뷰어(OSMD)와 플레이어 싱크 기능 (정밀 커서 추적 구현)
...
#### 3.2.1 백엔드 최적화
- [x] 음원 처리 캐싱 개선 (FastAPI-Cache2 + Redis/InMemory)
- [x] 응답 압축 (GzipMiddleware 적용)
- [x] 대용량 파일 스트리밍 처리 (Cache-Control 헤더 최적화)
- [ ] 비동기 작업 큐 개선 (현재 백그라운드 태스크 → Celery 고려)
- [x] 데이터베이스 쿼리 최적화 (N+1 문제 점검, SQLite WAL 모드 활성화)
- [x] Prometheus 모니터링 도입 (/metrics 엔드포인트)

#### 3.2.2 프론트엔드 최적화
- [ ] 이미지 최적화 (Next.js Image 컴포넌트 활용)
- [x] 코드 스플리팅 개선 (Next/Dynamic을 통한 뷰어 컴포넌트 지연 로딩)
- [x] API 데이터 캐싱 (React Query + Mix hashing 적용)
- [ ] 서비스 워커 캐싱 (PWA)
...
#### 3.3.1 린팅 및 포맷팅
- [x] Python: Black + isort + flake8 설정 완료
- [x] TypeScript: ESLint + Prettier 설정 완료
- [x] Pre-commit 훅 설정 (Backend)
